...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
        ...
        if not self.opt.asr:
            ...
            if self.opt.asr_model == 'ave':
                aud_features = torch.from_numpy(aud_features).unsqueeze(0)  # (1,2999,512)

                # support both [N, 16] labels and [N, 16, K] logits
                if len(aud_features.shape) == 3:
                    aud_features = aud_features.float().permute(1, 0, 2)  # [1, 2999, 512] --> [2999, 1, 512]
                ...
                print(f'[INFO] load {self.opt.aud} aud_features: {aud_features.shape}')
            audio_length = aud_features.shape[0]


            if self.opt.contrast_loss:
                rev_aud_features = np.load(opt.rev_aud)
                if self.opt.asr_model == 'ave':
                    rev_aud_features = torch.from_numpy(rev_aud_features).unsqueeze(0)  # (1,2999,512)
                    # support both [N, 16] labels and [N, 16, K] logits
                    if len(rev_aud_features.shape) == 3:
                        rev_aud_features = rev_aud_features.float().permute(1, 0, 2)  # [1, 2999, 512] --> [2999, 1, 512]
                else:
                    rev_aud_features = torch.from_numpy(rev_aud_features)
        ...
        if self.opt.au45:
            ...
            if self.opt.bs_area == "upper":
                ...
                bs = np.hstack((bs[:, 0].reshape(-1, 1), bs[:, 2].reshape(-1, 1), bs[:, 3].reshape(-1, 1),
                                bs[:, 8].reshape(-1, 1)))
        ...
        # # TODO 读取情绪标签
        if self.opt.emo and self.opt.aud == '':

            emo_labels = np.load(os.path.join(self.root_path, 'emotion_label.npy'),
                                 allow_pickle=True).item()  # 一个字典{id:label} id从0开始
        elif self.opt.emo:  # 读取音频文件所在目录下的标签
            # 提取目录路径
            # print('1111111111111111111111')
            directory_path = os.path.dirname(self.opt.aud)
            emo_labels = np.load(os.path.join(directory_path, 'emotion_label.npy'),
                                 allow_pickle=True).item()  # 一个字典{id:label} id从0开始
        ...
        for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
            ...
            if not self.opt.asr and self.opt.aud == '':
                ...
                self.auds.append(aud)
                # if self.opt.contrast_loss:
                #     rev_aud = rev_aud_features[min(f['aud_id'], rev_aud_features.shape[0] - 1)]
                #     self.rev_auds.append(rev_aud)
            ...
            # # TODO 取出没一帧的情绪标签
            if self.opt.emo and self.opt.aud == '':
                per_emo_label = emo_labels[f['img_id']]
                # print(per_emo_label)
                self.emo_labels.append(per_emo_label)
            elif self.opt.emo:  # 读取音频文件所在目录下的标签
                # 提取目录路径
                # print('1111111111111111111111')
                #
                if f['img_id'] <= audio_length:
                    per_emo_label = emo_labels[f['img_id']]
                    self.emo_labels.append(per_emo_label)
        # load pre-extracted background image (should be the same size as training image...)
        ...
        if self.opt.asr:
            ...
            # auds is novel, may have a different length with images
                ...
                self.auds = aud_features
            if self.opt.contrast_loss:
                self.rev_auds = rev_aud_features   # [N,1,512]
        ...
        # 提取情感标签
        if self.opt.emo:
            self.emo_labels = np.array(self.emo_labels, dtype=np.float32)  # [N]
            self.emo_labels = torch.from_numpy(self.emo_labels).view(-1, 1)  # [N, 1]
            print('emo_label_num:', self.emo_labels.shape)
        ...
        if self.preload > 1:
            ...
            if self.auds is not None:
                self.auds = self.auds.to(self.device)
                if self.opt.contrast_loss:
                    self.rev_auds = self.rev_auds.to(self.device)
            ...
            #########
            if self.opt.emo:
                self.emo_labels = self.emo_labels.to(self.device)
        # load intrinsics
    ...
    def collate(self, index):
        ...
        # TODO 读取处理好的emo信息
        if self.opt.emo:
            results['emo_label'] = self.emo_labels[index].to(self.device)  # [1]
