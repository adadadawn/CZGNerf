...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        if self.opt.contrast_loss:
            audneg = data['rev_aud'].to(self.device)  # now[8,1,512]
        ...
        if not self.opt.torso:
            outputs = self.model.render(rays_o, rays_d, auds, bg_coords, poses, emo_label=emo_label,eye=eye, index=index, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if (self.opt.patch_size <= 1 and not self.opt.train_camera) else True, **vars(self.opt))
        else:
        ...
        # TODO emo att loss
        # emo att loss
        if self.opt.amb_emo_loss and not self.opt.torso and not self.opt.finetune_lips:
        ...
        if self.global_step % 16 == 0 and not self.flip_finetune_lips:
            ...
            with torch.no_grad():
                     ...
                     sigmas_raw, rgbs_raw, ambient_aud_raw, ambient_eye_raw, ambient_emo_raw,unc_raw = self.model(xyzs, dirs,
                                                                                                  enc_a.detach(),
                                                                                                  ind_code.detach(), eye,
                                                                                                  emo_label=emo_label)

            sigmas_reg, rgbs_reg, ambient_aud_reg, ambient_eye_reg,ambient_emo_reg, unc_reg = self.model(xyzs + xyz_delta, dirs,
                                                                                              enc_a.detach(),
                                                                                              ind_code.detach(), eye,
                                                                                              emo_label=emo_label)
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, auds, bg_coords, poses, eye=eye,emo_label=emo_label, index=index, staged=True, bg_color=bg_color, perturb=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        outputs = self.model.render(rays_o, rays_d, auds, bg_coords, poses, emo_label = emo_label,eye=eye, index=index, staged=True, bg_color=bg_color, perturb=perturb, **vars(self.opt))
    ...
    def test(self, loader, save_path=None, name=None, write_image=False):
        ...
        if self.opt.video_aud != '':
            os.system(f'ffmpeg -i {os.path.join(save_path, f"{name}.mp4")} -i {self.opt.video_aud} -strict -2 -c:v copy {os.path.join(save_path, f"{name}_audio.mp4")} -y')
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth, pred_ambient_aud, pred_ambient_eye,pred_ambient_emo, pred_uncertainty, truths, loss, loss_raw = self.eval_step(data)
                ...
                if self.local_rank == 0:
                    ...
                    save_path_ambient_emo = os.path.join(self.workspace, 'validation', f'{name}_{self.local_step:04d}_emo.png')
