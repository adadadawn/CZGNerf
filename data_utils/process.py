...
def extract_audio_features(path, mode='ave'):
    ...
    if mode == 'ave':
        cmd = f'python data_utils/ave.py --input {path}'
        os.system(cmd)
...
#     # 这里通过 try-except 块处理了两种不同版本的 landmarks 类型，以适应不同版本的库。
#     try:
...
#
#
#     print(' use face_alignment ')
#     # 获取指定目录下所有以 '.jpg' 扩展名结尾的文件的路径列表。
#     image_paths = glob.glob(os.path.join(ori_imgs_dir, '*.jpg'))
#     # 在控制台中显示一个进度条，以展示遍历 image_paths 列表的进度。
#     for image_path in tqdm.tqdm(image_paths):
#         # cv2.IMREAD_UNCHANGED 表示以原始的通道格式读取。这样 input 将是一个包含图像信息的 NumPy 数组。
#         input = cv2.imread(image_path, cv2.IMREAD_UNCHANGED) # [H, W, 3]
#         # 将图像的颜色通道从 BGR 转换为 RGB 格式。face_alignment 库通常使用 RGB 格式的图像进行面部关键点提取。
...
def extract_landmarks(ori_imgs_dir,base_dir):  # 用openface
    print(f'[INFO] ===== extract face landmarks from {ori_imgs_dir} =====')
    # 读取CSV文件
    print('use openface')
    ...
    y_columns = [col for col in columns if col.startswith(' y_')]

    # 将每一帧的点写入文本文件
    frame_count = df['frame'].max()  # 获取帧数
    # print(frame_count)  # 1779
    for frame_number in range(1, frame_count+1):  # !! 从1开始计数，后面记得减回去
         frame_data = df[df['frame'] == frame_number]  # 获取当前帧的数据
         points = []
         for index, row in frame_data.iterrows():
             points.append([(row[x_col], row[y_col]) for x_col, y_col in zip(x_columns, y_columns)])  # 将x和y坐标组合成点
         points_array = np.array(points[0])  # 转换为NumPy数组，形状为[68, 2]
         # print(points_array.shape)
         # print(points_array)
         np.savetxt(f'{ori_imgs_dir}/{frame_number-1}.lms', points_array, '%f')
    print(f'[INFO] ===== extracted face landmarks =====')
#def extract_landmarks(ori_imgs_dir):
#     print(f'[INFO] ===== extract face landmarks from {ori_imgs_dir} =====')
#
...
#         input = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)  # [H, W, 3]
         # 将图像的颜色通道从 BGR 转换为 RGB 格式。face_alignment 库通常使用 RGB 格式的图像进行面部关键点提取。
#         input = cv2.cvtColor(input, cv2.COLOR_BGR2RGB)
#         lmk3d, mesh, pose = model.get_all_outputs(input)  # [1,3,68] list
#         if len(lmk3d) > 0:
#             lands = np.transpose(lmk3d[0], (1, 0))[:, :2]
             # print(lands)
             # print(lands.shape)
#             np.savetxt(image_path.replace('jpg', 'lms'), lands, '%f')
     # 释放资源
...
def extract_emotion_label(ori_img_path, outpath):
    print(f'[INFO] ===== extract emo_label to {outpath}.npy =====')
    idx = 0

    image_paths = glob.glob(os.path.join(ori_imgs_dir, '*.jpg')) # 一个后缀都是jpg的列表 ['/path/to/images/image1.jpg',.....]
    # print(image_paths)  # 文件夹里面是按顺序排好的
    emotion_mapping = {
        'happy': 7,
        'sad': 1,
        'surprise': 2,
    ...
    out_dict = {}    # id:emotion_label
    for per_img_path in image_paths:  # '/path/to/images/image1.jpg'
        image_name = os.path.basename(per_img_path)  # image1.jpg
        ...
        try:
            ...
            # 使用 DeepFace 分析情绪
            analysis = DeepFace.analyze(per_img_path, actions=['emotion'])
            dominant_label = analysis[0]['dominant_emotion']
            emotion_num = emotion_mapping[dominant_label]

            # 将文件名（不包括扩展名）和情绪标签添加到字典中
            # os.path.splitext(image_name)[0] -> image1
            # emotion_num_np = np.array(emotion_num) # 转为np类型
            # print(emotion_num_np)
            out_dict[int(os.path.splitext(image_name)[0])] = emotion_num
            # print(out_dict)  {0: 7, 1: 6, 10: 6, 100: 1}
        except Exception as e:
            print(f"An error occurred while analyzing {image_name}: {e}")
        idx += 1
        if idx % 200 == 0:
            print(idx)

    np.save(outpath+'/emotion_label.npy', out_dict)
    print(f'[INFO] ===== extracted emotion_dict =====')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('path', type=str, help="path to video file")
    parser.add_argument('--task', type=int, default=-1, help="-1 means all")
    parser.add_argument('--asr', type=str, default='ave', help="ave or hubert")
    ...
    if opt.task == -1 or opt.task == 7:
        extract_landmarks(ori_imgs_dir,base_dir=base_dir)
